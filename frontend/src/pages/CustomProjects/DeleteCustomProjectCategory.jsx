...
//The Main function
export default function DeleteCustPCategory(props) {
  ...
  const { openPopupDeleteCustPCategory, setOpenPopupDeleteCustPCategory } = props;

  const apiUrl = `http://localhost:8070/CustPCategories/deleteCustPCategory/${props.CUPCID}`; // Change to your API URL
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    if (props.categoryName == values.categoryName) {
        try {
            ...
            sessionStorage.setItem("CustPCategoryDeleted", "1");
        ...
        setSubmitting(false);
        setOpenPopupDeleteCustPCategory(false);
        }
    } else {
        setNotify({
            isOpen: true,
  ...
  return (
    <Dialog
      open={openPopupDeleteCustPCategory}
      onBackdropClick={() => setOpenPopupDeleteCustPCategory(false)}
      maxWidth="sm"
