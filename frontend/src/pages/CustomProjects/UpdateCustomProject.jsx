...
//The Main function
export default function UpdateCustProject(props) {
  ...
  const apiUrl = `http://localhost:8070/CustomProjects/updateCustomProject/${props.CUPID}`; // Change to your API URL

  const navigate = useNavigate();
  const { openPopupUpdateCustProject, setOpenPopupUpdateCustProject } = props;
  const [fetchedCustProjectData, setFetchedCustProjectData] = useState();
  const [loading, setLoading] = useState(true);
  const [projectManagers, setProjectManagers] = useState([]);
  const [teamMembers, setTeamMembers] = useState([]);
  const [CustPCategory,setCustPCategory] = useState([]);

  const formattedStartDate = fetchedCustProjectData?.startDate
  ? new Date(fetchedCustProjectData.startDate).toISOString().split('T')[0]
  : '';

  const formattedDeadline = fetchedCustProjectData?.deadline
  ? new Date(fetchedCustProjectData.deadline).toISOString().split('T')[0]
  : '';
  ...
  useEffect(() => {
    ...
    async function getCustProject() {
        setLoading(true);
        await axios
            .get(`http://localhost:8070/CustomProjects/getCustomProject/${props.CUPID}`)
            .then((res) => {
                setFetchedCustProjectData(res.data);
            })
            .catch((err) => {
                console.log(err);
            })
    ...
    }, [props, openPopupUpdateCustProject]);
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
      try {
        ...
        sessionStorage.setItem("CustProjectUpdated", "1");
        ...
        setSubmitting(false);
        setOpenPopupUpdateCustProject(false);
      }
    };

  return (
    <Dialog
      open={openPopupUpdateCustProject}
      onBackdropClick={() => setOpenPopupUpdateCustProject(false)}
      maxWidth="md"
      ...
      <div className="popup">
        ...
        <div className="d-flex justify-content-between align-items-center">
          <div className="d-flex align-items-center">
            <p className="popupTitle">Update Custom Project</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <Formik
            initialValues={{
                projectName: fetchedCustProjectData?.projectName || "",
                description: fetchedCustProjectData?.description || "",
                startDate: formattedStartDate || "",
                deadline: formattedDeadline|| "",
                priority: fetchedCustProjectData?.priority || "",
                projectManagers: fetchedCustProjectData?.projectManagers || [],
                teamMembers: fetchedCustProjectData?.teamMembers || [],
                ProjectCategory: fetchedCustProjectData?.ProjectCategory || "",
                projectBudget: fetchedCustProjectData?.projectBudget || "",
                attachments: fetchedCustProjectData?.attachments || [],
                status: fetchedCustProjectData?.status || "",
                notes: fetchedCustProjectData?.notes || "",
            }}
