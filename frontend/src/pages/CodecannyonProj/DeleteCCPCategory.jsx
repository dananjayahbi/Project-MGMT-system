...
//The Main function
export default function DeleteCCPCategory(props) {
  ...
  const { openPopupDeleteCCPCategory, setOpenPopupDeleteCCPCategory } = props;

  const apiUrl = `http://localhost:8070/CCPCategories/deleteCCPCategory/${props.CCPCID}`; // Change to your API URL
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    if (props.categoryName == values.categoryName) {
        try {
            ...
            sessionStorage.setItem("CCPCategoryDeleted", "1");
        ...
        setSubmitting(false);
        setOpenPopupDeleteCCPCategory(false);
        }
    } else {
        setNotify({
            isOpen: true,
  ...
  return (
    <Dialog
      open={openPopupDeleteCCPCategory}
      onBackdropClick={() => setOpenPopupDeleteCCPCategory(false)}
      maxWidth="sm"
