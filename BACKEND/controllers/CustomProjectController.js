const CustomProjects = require("../models/CustomProject.model");

// Function to generate a random PID
function generateRandomPID() {
  const prefix = 'CP'; // First 2 letters
...
const addProject = async (req, res) => {
  ...
  do {
      ...
      existingProject = await CustomProjects.findOne({
          PID: pid,
      });
  } while (existingProject);

  project = await CustomProjects.create({
    PID: pid,
    projectName,
    description,
    startDate,
...
const getAllProjects = async (req, res) => {
    const abc = await CustomProjects.find()
      .then((projects) => {
        res.json(projects);
      })
      .catch((e) => {
...
const getProject = async (req, res) => {
    try {
      const projectObject = await CustomProjects.findById(req.params.id);
...
const updateProject = async (req, res) => {
    try {
      ...
      // Updating
      const update = await CustomProjects.findByIdAndUpdate(req.params.id, updateData);
...
const deleteProject = async (req, res) => {
    try {
      const deleted = await CustomProjects.findByIdAndDelete(req.params.id);
